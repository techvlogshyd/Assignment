...
|---------|---------|-------|
| `ARTIFACTS_ROOT` | repo-root `test-results/` (resolved from `main.py`, not CWD) / `/data/test-results` (in compose) | Where to scan for `junit*.xml` and `playwright-report.json` |
