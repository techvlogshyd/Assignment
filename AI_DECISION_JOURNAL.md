...
4. **"Split Playwright CI from the deliberate red demo."**
   The brief requires a deliberately failing automation-framework/UI demo for the Monday triage story but CI must stay mergeable. I tagged that case with **`@pytest.mark.demo_intentional_fail`** and excluded it from CI via **`python -m pytest -m "not demo_intentional_fail"`**, leaving **`python -m pytest`** (no `-m`) for the full demo. Prefer **`python3 -m pytest`** locally when the `pytest` launcher is not on `PATH`. **`AUTOMATION_APP`** (legacy **`E2E_CUSTOMER`**) selects the app bundle under **`automation-framework/apps/`**. The dashboard's history table makes this triage demo concrete: the failing test shows up as "newly failing" once the demo run is ingested.
