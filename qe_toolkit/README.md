...
| `qe_toolkit.junit` | `automation-framework/dashboard/main.py`, `scripts/flag_flakes_from_junit.py` | `parse_junit_files(root)` and `find_flakes(root)` — works on any JUnit XML |
| `qe_toolkit.playwright` | `automation-framework/dashboard/main.py` | `parse_playwright_json(path, artifacts_root)` — Playwright JSON reporter parser with attachment URL rewriting |
| `qe_toolkit.text` | `qe_toolkit.playwright`, `automation-framework/dashboard/main.py` | `strip_ansi` — removes terminal colour codes from Playwright error messages for HTML/JSON |
